...
std::unordered_map<int, ObstacleBox> g_ObstacleBox;
...
private:
    ...
    void HandleCommand(const std::string& line)
    {
        ...
        else if (cmd == "OBSTACLE_SPAWN")
        {
            ...
            ObstacleBox b;
            b.sessionID = m_SessionKey;
            b.key = g_ObstacleID++;
            b.x = x;
            b.z = z;


            for (auto it = g_ObstacleBox.begin(); it != g_ObstacleBox.end();)
            {
                //같은 위치에 있으면
                if (it->second.x == x && it->second.z == z)
                {
                    return;
                }
                else
            ...
            g_ObstacleBox[b.key] = b;

            std::cout << "[OBSTACLE_SPAWN] key=" << m_SessionKey
                << " (" << x << "," << z << ")\n";
        ...
        else if (cmd == "OBSTACLE_DELETE")
        {
            ...
            for (auto it = g_ObstacleBox.begin(); it != g_ObstacleBox.end();)
            {
                if (it->second.x == x && it->second.z == z && it->second.sessionID == m_SessionKey)
                {
                    it = g_ObstacleBox.erase(it);
                    std::cout << "[OBSTACLE_DELETE] key=" << m_SessionKey
                        << " (" << x << "," << z << ")\n";
...
int main()
{
    try
    {
        ...
        //tcp::acceptor acceptor(io, tcp::endpoint(boost::asio::ip::make_address("172.21.1.29"), 8080));
