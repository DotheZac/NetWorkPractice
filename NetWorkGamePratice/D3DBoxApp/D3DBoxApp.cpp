...
struct App
{
    ...
    bool Init(HWND hWnd)
    {
        ...
        //m_Client = std::make_shared<AsyncClient>(*m_IO, "172.21.1.35", 8080);
    ...
    void ProcessNetwork()
    {
        ...
        while (m_Client->PopLine(line))
        {
            ...
            else if (cmd == "OBSTACLE_SPAWN")
            {
                ...
                //for (auto it = m_ObstacleBoxes.begin(); it != m_ObstacleBoxes.end();)
                //{
                //    if (it->m_Pos == pos)
                //    {
                //        return;
    ...
    {
        int cellX = static_cast<int>((cellCenter.x / m_CellSize) - 0.5f);
        int cellZ = static_cast<int>((cellCenter.z / m_CellSize) - 0.5f);
    ...
    void SendDeleteRequestToServer(const Vector3& cellCenter)
    {
        int cellX = static_cast<int>((cellCenter.x / m_CellSize) - 0.5f);
        int cellZ = static_cast<int>((cellCenter.z / m_CellSize) - 0.5f);
    ...
    {
        int cellX = static_cast<int>((m_Boxes[m_MySessionKey].m_Pos.x / m_CellSize) - 0.5f);
        int cellZ = static_cast<int>((m_Boxes[m_MySessionKey].m_Pos.z / m_CellSize) - 0.5f);
    ...
    void SendMoveRequestToServer(const Vector3& cellCenter)
    {
        ...
        if (float(cellX) + 0.5f == m_Boxes[m_MySessionKey].m_Pos.x && float(cellZ) + 0.5f == m_Boxes[m_MySessionKey].m_Pos.z)
        {
            SendDeleteRequestToServer(cellCenter);
            return;
        }
    ...
    void SendObstacleSpawnRequestToServer(const Vector3& cellCenter)
    {
        int cellX = static_cast<int>((cellCenter.x / m_CellSize) - 0.5f);
        int cellZ = static_cast<int>((cellCenter.z / m_CellSize) - 0.5f);
    ...
    void SendObstacleDeleteRequestToServer(const Vector3& cellCenter)
    {
        int cellX = static_cast<int>((cellCenter.x / m_CellSize) - 0.5f);
        int cellZ = static_cast<int>((cellCenter.z / m_CellSize) - 0.5f);
    ...
    void ToggleObstacleAtMouse(int mx, int my)
    {
        ...
        if (grid[gz][gx] == 0)
        {
            ...
            SendObstacleSpawnRequestToServer(cellPos);
        }
        else
        {
            // Remove obstacle
            ...
            //        }),
            //    m_ObstacleBoxes.end());
            SendObstacleDeleteRequestToServer(cellPos);
        }
    }
